...
class TestOidcOpFrontend(object):
    ...
    def insert_client_in_client_db(self, frontend, **kwargs):
        ...
        frontend.app.storage.store("client_info", key=client_conf['client_id'], value=client_conf)
        client = frontend.app.storage.fetch("client_info", key=client_conf['client_id'])
        return client
    ...
    def test_handle_authn_request_faulty(self, context, frontend, authn_req):
        ...
        assert json.loads(res.message)[
                   'error_description'] == "Missing required attribute 'redirect_uri'"
    ...
    def setup_for_authn_response(self, context, frontend, auth_req):
        ...
        auth_info = AuthenticationInformation(
            "urn:oasis:names:tc:SAML:2.0:ac:classes:Password", "2021-09-30T12:21:37Z",
            "unittest_idp.xml"
        )
